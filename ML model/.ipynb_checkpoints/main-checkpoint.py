...
def main():
    ...
    for r in range(4, 5):
        ...
        for idx_combo in combinations(range(4), r):
            ...
            study.optimize(optuna_objective, n_trials=N_TRIALS, catch=(ValueError, FloatingPointError))
            # --------------------------------------------------
            # Trial results
            # --------------------------------------------------
            trial_data = []
            ...
            train_loader = prepare_dataloader(X_tr, y_tr, batch_size=batch_size)
            ...
            with pd.ExcelWriter(output_file) as writer:
                ...
                best_factor_df.to_excel(writer, sheet_name="SHAP", index=False)
